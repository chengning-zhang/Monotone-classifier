class MonotoneClassifier():
  ...
  1. EM algorithm taking into missing covariates 2. delete missing cases using complete cases 3. mean imputation 4. median imputation
  
  Parameters
  ----------
  Note : empirical best stopping rule is (1e-4, 100) # this may not be true, since true point mass 1/m, initial point mass also 1/m
  ...
  handle : str in ['missing', 'complete', 'mean', 'median']
    'missing': Use EM to handle missing X
    'complete': delete incomplete examples, causing bias in MAR
    'mean': impute missing values by mean of that column
    'median':impute missing values by median of that column

  random_init : bool
    Whether randomly intialize point mass of U and X.

  init_low, init_high : int
    controls the range of randomly intialized point mass of U and X
  ...
  def __init__(self, tol = 1e-07, max_iter = 300, verbose = False, handle = 'missing', random_init = True, init_low = 1, init_high = 4):
      self.tol = tol
  ...
  def fit(self, X, Y):
      ...
      if self.handle == 'missing': # no need to process X, if no missing X, should be same as 'complete'
        pass

      elif self.handle == 'complete':# process X by filtering complete cases
        idx_complete = [i for i,x in enumerate(X) if not any(np.isnan(x))]  
        X = X[idx_complete,:]; 
        Y = Y[idx_complete]

      elif self.handle == 'mean': # impute X by mean 
        imp = SimpleImputer(missing_values=np.nan, strategy='mean').fit(X)
        X = imp.transform(X)

      elif self.handle == 'median': # impute X by median
      ...
      # NPEM training
      if self.random_init: # random init of U and X
        np.random.seed(seed = self.low * self.high)
        p = np.random.randint(low =self.low, high=self.high, size = self.m_); p = p/sum(p)
        hashmap_U = dict(zip(self.uniqueObservedX_, p ))
        hashmap_X = dict(zip(self.uniqueObservedX_, p ))
      else:
  ...
  def _NPEM(self, hashmap_U, hashmap_X, X, Y):
      ...
      log_likelihood = []
      while True:
          # R_j is the [cdf_u(x1),...cdf_u(xm)] = [P(D=1|x1),...P(D=1|xm)], uniqueObservedX_ includes all fully observed X and augmented X. 
          R_j = {ele: self._CDF(ele, prev_U) for ele in self.uniqueObservedX_}; # x1,....xm are fully observed, cdf(x1) ,... cdf(xm) no problem
          #log_likelihood
          ...
          # Make sure point mass sum to 1!! When xi = [3,3,3](the biggest one), di = 0, I : cannot get likelihood II : sum of point mass !=1
          if np.abs(sum(np.array(list(cur_U.values()) ) ) - 1) > 1e-4:
              pass
          iter +=1
          error1 = sum( ( np.array(list(cur_U.values())) - np.array(list(prev_U.values())) )**2) / len(cur_U) # make sure normalize error 
          error2 = sum( ( np.array(list(cur_X.values())) - np.array(list(prev_X.values())) )**2) / len(cur_X)
  ...
  def _likelihood(self, R_j, prev_U, prev_X, X, Y, iter):
      ...
      ----------
      R_j : dict 
        The cdf of at current iteration j, [cdf_u(x1), .... cdf_u(xm)]
      ...
      for i,xi in enumerate(X):
          if Y[i] == 1:
              ...
              if not any(np.isnan(xi)): # if xi is complete
                add_on = prev_X[tuple(xi)]* (1-R_j[tuple(xi)]) ## !!!! In the first iteration of NPEM, this "add_on" is always <= 0 
              else:
  ...
  def predict_proba(self, X):
      ...
      note
        Case 1 : x is fully observed,  P(D=1|x) = R(x) = cdf_u(x) = sum_{xk <= x, xk in uniqueObservedX_}.
        Case 2 : x is missing but missing pattern seen in training, P(D=1|x) = frac{ sum_{xl. in uniqueObservedX_ , M(xl) = x} R(xl)*p(xl) } { sum_{xl. in uniqueObservedX_ , M(xl) = x} p(xl). }
        Case 3 : x is missing but missing pattern not seen,  it is possible  augmented(x) not in  uniqueObservedX_,     sum_{xl in uniqueObservedX_ , M(xl) = x} p(xl) = 0, 
        
      Return
      -------
      T : array-like of shape, case3 return nan
      """
      ...
      for x in X:
          if not any(np.isnan(x)): # fully observed
            ret.append( self._CDF(x, self.dict_U_) )
          
          elif self._hashable_x(x) in self.C_: # missing but with seen pattern
            numerator = sum([self._CDF(xl, self.dict_U_) * self.dict_X_[xl]  for xl in self.C_[self._hashable_x(x)]] )
            denominator = sum([self.dict_X_[xl]  for xl in self.C_[self._hashable_x(x)]])
  ...
  def predict(self, X):
      ...
      for p in prob:
        if p>=0.5:
          ...
          ans.append(np.nan)
      return np.array(ans)
      #np.array([1 if p > 0.5 else 0 for p in prob]) 
  
  
  def _CDF(self, x, hashmap):
      """
      helper function to calculate the CDF of U at x given point mass hashmap, x is fully observed!!! i.e P(D=1|x) = R(x) = CDF_U(x) =  sum_{x_k <= x} P(U = xk)
      ...
      hashmap : self.dict_U_
        Given point mass for calculating CDF

      Returns
      -------
      ans : float
        CDF at x, sum_{x_k <= x, x_k in uniqueObservedX_ } P(U = xk)
      """
